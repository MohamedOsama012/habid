...
class UserCubit extends Cubit<AuthState> {
  UserCubit({required this.authRepoImpl}) : super(AuthInitial());
  ...
  Future<void> register(
    ...
    emit(AuthLoading());
    try {
      var result = await authRepoImpl.register(emial, password);
      if (result == "success") {
  ...
  Future<void> logIN({required String emial, required String password}) async {
    emit(AuthLoading());
    try {
      var result = await authRepoImpl.signIn(emial, password);
...
//!forget password
  Future<void> forgetPassword({required String emial}) async {
    ...
    try {
      var result = await authRepoImpl.forgetPassword(emial);
  ...
  Future<void> getUserData() async {
    await authRepoImpl.getUserData();
  }
